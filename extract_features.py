...
Features used:
1. Is the position considered as endgame (see utils.is_endgame)
2. The side to move.
3. Castling rights.
4. The count of each type of piece.
5. The location of each piece and value of the lowest-valued attacker
   and defender of the piece.
6. The distance each sliding piece, {bishop, rook, queen}, can legally
   move in each direction.
7. The value of the lowest-valued attacker and defender of each square.
...
def _is_endgame(position, verbose=False):
    '''
    True if it's considerd an endgame position.

    Number of features contributed: 1.
    '''
    result = [is_endgame(position)]
    if verbose:
        print()
        print('Is endgame?')
        print('------------------------------------')
