...
void uci_loop(char* engine_name, char* engine_version, char* engine_author) {
    ...
    // my add - create file and print headline
    data_print_headline();
    // end my add
...
void* execute_uci_go(void* pv_line)
{
    ...
    set_game_settings_max_nodes(&game_settings); // my add
    set_game_search_bpruning(&main_game, 1);    // my add
    set_game_settings_normal_move_time(&game_settings);       // my add
    ...
    //my add
    char fen[1024];
    util_get_board_fen(&main_game.board, fen);
    // end my add
    ...
    // my add - save search data to file
    data_print_search(fen);
